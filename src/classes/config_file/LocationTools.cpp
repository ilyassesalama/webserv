...
std::string getLocationPath( std::string file, size_t *startIndex ) {
	...
	if (!isspace(file[i]) && file[i] != '{') throw ConfigurationFile::customException((char *)"Error, there is a syntax error");
...
size_t getLocationEnd( std::string file ) {
	...
	if (endPos == std::string::npos) throw ConfigurationFile::customException((char *)"Error, there is a syntax error");
...
void skipRoute(std::string file, size_t *startIndex) {
	size_t i = file.find("}");
	if (i == std::string::npos) throw ConfigurationFile::customException((char *)"Error, there is a syntax error");
	*startIndex += ++i;
}
...
void checkCGIMethod( std::vector<std::string> values ) {
	...
	for (it = values.begin(); it != values.end(); it++ ) {
		...
		else throw ConfigurationFile::customException((char *)"Error, (cgi_methods): method either not allowed or duplicated");
	}
}
...
void checkAllowedMethods(std::vector<std::string>allowed_methods) {
	...
	for (it = allowed_methods.begin(); it != allowed_methods.end(); it++) {
		...
		else throw ConfigurationFile::customException((char *)"Error, (allowed_methods): method either not allowed or duplicated");
	}
}