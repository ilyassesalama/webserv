...
void RequestParser::setParsingState(bool isOK){
    this->parsingState.ok = isOK;
}
...
void RequestParser::setStatusCode(int statusCode) {
    this->parsingState.statusCode = statusCode;
}
...
bool RequestParser::isPathAccessible() {
    ...
    if(route == NULL) {
        ...
        if(this->route->path != "/")
            this->requestResourcePath.append(getRequestLine()["path"].substr(getRequestLine()["path"].find(location) + location.length()));
        else
            this->requestResourcePath.append(getRequestLine()["path"]);
